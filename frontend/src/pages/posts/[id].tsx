...
export default function PostDetails() {
  ...
  function canModifyPost(): boolean {
    return user && post && post.author && post.author.username === user.username;
  }
  ...
  return (
    <>
      ...
      <div className="card">
        {post.imagePath && (
          <img
            src={`${process.env.NEXT_PUBLIC_S3_URL}/${post.imagePath}`}
        ...
        <div className="mt-4">
          ...
          {post.replies.map((reply) => (
            <div key={reply.id} className="card mb-3">
              {reply.imagePath && (
                <img
                  src={`${process.env.NEXT_PUBLIC_S3_URL}/${reply.imagePath}`}
                  className="card-img-top"
                  alt="Reply image"
